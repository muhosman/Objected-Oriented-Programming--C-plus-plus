...
public:
	...
	HEX(int size_Value,int flag_value);
	HEX(int size_Value);
	HEX();
	...
	bool compare_two_hex_game(class HEX Game2);
	void test_value();
	...
	char get_alphabet(int input) const
	{
			...
			case 3:
				do {
					...
					if (input1>=0 && input1<game_counter && input2>=0 && input2<game_counter && input1 != input2) {
						if(GAME[input1].compare_two_hex_game(GAME[input2]))
...
void HEX::play_Game()
{
	...
	if (get_flag() != 1) {
		do{
				//Mover Player 1
				do{
					cout<<"PLAYER 1 // Enter The Column and The Row or LOAD or SAVE or MENU command line(EXMP : C7):";
				...
				{
					do{
						cout<<"PLAYER 2 //  Enter The Column and The Row or LOAD or SAVE or MENU command line(EXMP : C7):";
						operation_control(row,col);
				...
				if(flag != 1)
					for(int i = 0;i<get_size();i++){
						if('O' == hexCells[0][i].get_type()){
							...
							if(get_flag() == 1&&get_game_style() == 2){
								cout<<"\nComputer win ! \n";
								print_bord();
								break;
							}
							if(get_flag() == 1&&get_game_style ()== 1){
								cout<<"\nPlayer 2 win ! \n";
								print_bord();
								break;
							}
...
void HEX::print_bord() const
{
	...
	for (int i = 0; i < get_size(); i++)
	{
		...
		for (int j = 0; j < get_size(); j++)
		{
			std::cout << hexCells[i][j].get_type() <<" ";
		}
		std::cout << endl;
	}
}
...
bool HEX::coordinate_operation(std::string command,int &row,char &col)
{
	int first,last;
	std::string roww;
...
bool HEX::player_move(int input,int &row,char &col)
{
	...
	{
		for (int j = 0; j < get_size(); j++) {
			if(col == hexCells[i][j].get_column() && row >=0 && row <= get_size() && row == hexCells[i][j].get_row() && hexCells[i][j].get_type() == '.'){
	...
	if(operation == 0)
	{
		ofstream savefile(save_name);
		  ...
		  if (savefile.is_open())
		  {
		    ...
		    savefile << "Size:"<<get_size()<<endl;
		    savefile << "Board:\n";
		    for(int i = 0; i<size ; ++i)
		    {
		    	for(int j = 0; j<size ; ++j)
		    	{
		    		savefile<<hexCells[i][j].get_type();
		    	}
		    	savefile<<endl;
		    }
		  ...
		  {counter = 0;
		    while ( getline (loadfile,line) )
		    {
		    	...
		    	if( (counter<4 && game_style == 1)  || (game_style == 2 && counter < 6)){
		    		...
		    		{
							if (line[i] == '.') {
								hexCells[temp][i].set_cell(1);
							}
							else if (line[i] == 'X') {
								hexCells[temp][i].set_cell(3);
							}
							else if (line[i] == 'O') {
								hexCells[temp][i].set_cell(4);
							}
		    		}
		    		temp++;
		    	}
...
bool HEX::compare_two_hex_game(class HEX Game2)
{
	if (get_marked_specific()>Game2.get_marked_specific())
		return 1;
